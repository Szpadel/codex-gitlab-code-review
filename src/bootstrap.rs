...
pub(crate) struct BootstrappedRuntime {
    pub(crate) config: Config,
    pub(crate) run_once: bool,
    pub(crate) service: Arc<ReviewService>,
    pub(crate) status_service: Arc<StatusService>,
    pub(crate) gitlab_discovery_mcp: Option<Arc<GitLabDiscoveryMcpService>>,
    pub(crate) dev_tools: Option<Arc<DevToolsService>>,
}
...
pub(crate) async fn bootstrap_runtime(
    ...
    bootstrap_runtime_with_probe(config, options, &readiness_probe).await
}

async fn bootstrap_runtime_with_probe(
    mut config: Config,
    options: BootstrapOptions,
    ...
    let runtime = if options.dev_mode {
...
async fn build_normal_runtime(
    ...
    let bot_user_id = resolve_bot_user_id(config, &gitlab_client).await?;
...
pub(crate) async fn resolve_created_after(
    config: &Config,
