...
mod tests {
    ...
    #[tokio::test]
    async fn legacy_dry_run_comment_history_without_gitlab_markers_does_not_skip_same_sha()
    -> Result<()> {
        let mut config = test_config();
        config.feature_flags.gitlab_inline_review_comments = true;
    ...
    #[tokio::test]
    async fn inline_review_comments_fallback_when_head_sha_no_longer_matches_latest_diff()
    -> Result<()> {
        let mut config = test_config();
        config.feature_flags.gitlab_inline_review_comments = true;
